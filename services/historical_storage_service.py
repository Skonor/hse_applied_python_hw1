...
class HistoricalStorageService:
    ...
    def get_seasonal_stats(self, city):
        return self.seasonal_stats[self.seasonal_stats['city'] == city]
    
    def is_anomaly(self, city, season, temp):
        if temp < (self.seasonal_stats[(self.seasonal_stats['city'] == city) & (self.seasonal_stats['season'] == season)]['mean_temp'].values[0] - 2 * self.seasonal_stats[(self.seasonal_stats['city'] == city) & (self.seasonal_stats['season'] == season)]['std_temp'].values[0]) or \
           temp > (self.seasonal_stats[(self.seasonal_stats['city'] == city) & (self.seasonal_stats['season'] == season)]['mean_temp'].values[0] + 2 * self.seasonal_stats[(self.seasonal_stats['city'] == city) & (self.seasonal_stats['season'] == season)]['std_temp'].values[0]):
            return True
        else:
            return False
